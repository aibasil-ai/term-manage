...
- [ ] 權限最小化（目前實際使用：`storage`、`activeTab`、`scripting`、`sidePanel`、`host_permissions`）
...
- [ ] 針對各權限填寫用途說明（尤其是 `scripting`、`activeTab`、`host_permissions`）
...
- `host_permissions (http/https)`: 允許在一般網站執行文字帶入。
