...
- 本專案使用 `activeTab` + `scripting`，並宣告網站存取權限 `http://*/*`、`https://*/*`，用於在一般網站分頁執行文字帶入。
- 文字帶入僅在你明確操作擴充功能時執行（例如在介面點擊「帶入」或使用已設定的快捷鍵命令），不會在背景持續掃描網頁內容。
...
- `docs/release/reviewer-note-host-permissions.zh-TW.md`
